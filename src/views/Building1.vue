...
<script>
    ...
    export default {
        ...
        methods: {
            downloadData() {
                axios.get("http://4bcc0e78.ngrok.io/budynek1/" + this.parametr + "?timestamp_begin=" + this.data_od + "&timestamp_end=" + this.data_do)
                    .then(res => (this.history = res.data));
                return true
            }
        },
        created() {
            axios.get("http://4bcc0e78.ngrok.io/budynek1")
                .then(res => (this.actual = res.data));
            axios.get("http://4bcc0e78.ngrok.io/budynek1/U_b?timestamp_begin=1999-01-07&timestamp_end=2019-12-30")
                .then(res => (this.history = res.data));
        }
    }
</script>
